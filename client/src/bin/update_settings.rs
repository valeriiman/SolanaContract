...
fn main() {
    ...
    let program_info = client.get_account(&contract).unwrap();
    println!("{program_info:?}");

    // let y = Managment::try_from_slice(&program_info.data).unwrap();
    // println!("{y:?}");


    // let account = client.get_account_data(&contract);
    // let account = client.get_program_accounts(&contract);
    // println!("{account:?}");
}